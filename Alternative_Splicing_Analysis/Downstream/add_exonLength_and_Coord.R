...
add_psi_counts_per_repl <- function(event, rmats_dir, suffix) {
  ...
  for (li in 1:nrow(t) ){
    ## Create new columns for each replicate's PSI-value
      ...
      t$IncLevel2_r4[li]  <- as.numeric(unlist(strsplit(t$IncLevel2[li],","))[4])
  }
...
add_exonLen_and_coord <- function(splice_event, tab_suffix=".MATS.JCEC.txt", in_dir, out_dir, C1_reps, C2_reps) {
  ...
  t <- add_psi_counts_per_repl(splice_event,in_dir,tab_suffix, C1_reps, C2_reps)
  ...
  write_delim(t, file=paste0(out_dir, "/", splice_event , new_suffix), delim="\t",
